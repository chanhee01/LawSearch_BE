package com.example.lawSearch.domain.suggestion.exception;

import com.example.lawSearch.global.exception.ErrorCode;
import com.example.lawSearch.global.exception.ServiceException;

public class SuggestionNotFoundException extends ServiceException {

    private static final ErrorCode ERROR_CODE = ErrorCode.SUGGESTION_NOT_FOUND;
    private static final String MESSAGE_KEY = "해당 정책 건의가 존재하지 않습니다.";

    public SuggestionNotFoundException(Long suggestionId) {
        super(ERROR_CODE, MESSAGE_KEY, new Object[] {suggestionId});
    }
}
